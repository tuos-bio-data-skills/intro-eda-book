...
There is one idea about functions that every R user needs to understand if they are to avoid confusion. It relates to how functions modify their arguments, or more accurately, how they **do not** modify their arguments. Take a look at this example:
...
We created a variable `myvar` with the value 3.7, rounded this to a whole number with `round`, printed out the result, and then printed the value of `myvar`. Notice that **the value of `myvar` has not changed** after using it as an argument to `round`.
...
If we had intended to round the value of `myvar` so that we can use this new value later on, we have to assign the result of function evaluation, like this:
...
These two lines calculate the square root of the number 10 and assigned the result to `myvar`, then round this to a whole number and print the result. This method of linking steps together assigns a name to the result at each step, and then uses this as the argument to a function in the next step.
...
Notive that there aren't really any new ideas here. We have already seen that the R interpreter evaluates whatever is on the right hand side of the `=` symbol first before associating the resulting value with the appropriate argument name.
...
Nested function calls can be useful because they make R code less verbose (we write less) but this comes at a significant cost of reduced readability. No reasonable person would say that `round(x = sqrt(x = abs(x = -10)), digits = 0)` is easy to read! For this reason, we aim to keep function nesting to a minimum in this book. We will occasionally have to work with the nesting construct though, so it is important to understand how it works even if we don't like using it.
