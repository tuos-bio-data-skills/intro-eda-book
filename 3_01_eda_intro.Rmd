...
When we say that 'EDA is concerned with exploring the properties samples,' we actually mean that EDA is concerned with the properties of **variables** in one or more samples. In fact, we can be even more precise---property we are alluding to is the variable's **sample distribution**. The distribution of a variable describes the relative frequency with which different values occur. This is best understood by example...
...
Statisticians have devised different ways to quantify an association between variables. The common measures calculate some kind of **correlation coefficient**. The terms 'association' and 'correlation' are closely related; so much so they are often used interchangeably. Strictly speaking, correlation has a narrower definition: a correlation is defined by a metric (the 'correlation coefficient') that quantifies the degree to which an association tends to a certain pattern.
