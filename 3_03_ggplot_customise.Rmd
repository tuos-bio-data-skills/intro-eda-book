...
ggplot(penguins, 
       # vary colour by species
       aes(x = bill_length_mm, y = bill_depth_mm, colour = species)) + 
  # add points layer and customise -ppearance of points
...
-   the `TYP` part refers to the kind of scale we want to define.
...
The axes labels are a feature of the whole plot---i.e. they do not belong to a particular layer. This means we can't alter axis labels by passing arguments to the function that built a layer (`geom_point`). We have to use a labelling function. One way to set the axis labels uses the `xlab` and `ylab`:
...
ggplot(penguins, 
  ...
  # set the the x and y-axis labels using `xlab` and `ylab`
...
The `labs` function is a flexible alternative to `xlab` and `ylab`. It's more flexible because `labs` can be used to change the label of every mapped aesthetic. For example, if we want to set the labels of the x and y axes and the label associated with `species`, we use:
