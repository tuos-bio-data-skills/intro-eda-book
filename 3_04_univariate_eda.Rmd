...
We only looked at the first 50 values of the `flipper_length_mm` and `body_mass_g` variables because the `penguins` data set is a bit too big to view everything at once easily. It's hard to say much about the sample distribution of the two variables by just looking at a small subset of values. Because the data are sorted by island and species, those first 50 values are also not representative of the whole sample.
...
1.  Take the range of the variable and divide this into equal-sized, non-overlapping intervals. These are the 'bins'. We can use any interval size as long as the resulting bins (typically) include a few observations.
...
2.  the range of the data, which is about 60 mm; and
...
Look at the text below the `----`. This shows that `geom_histogram` adds a stat to the layer: `stat_bin`. This means **ggplot2** is going to take the raw `flipper_length_mm` data and bin it for us.
...
This is not quite the same as the example we saw above because it uses different bins. There is nothing special about the **ggplot2** default. It is usually a good idea to play around with the bin size to arrive at an informative histogram. We set the properties of the `geom_histogram` to tweak this kind of thing---the `binwidth` argument adjusts the width of the bins used.

Let's construct the histogram again with 5 mm wide bins, as well as adjust the colour scheme and axis labels a bit:
...
Whether or not that colour scheme is an improvement is a matter of taste. Mostly we wanted to demonstrate how the `fill` and `colour` arguments change the output--- `fill` sets the fill colour of the bar and `colour` deals with the line around each bar.

Notice that the effect of increasing the bin width is to 'smooth' the histogram, i.e. this version looks less jagged than the last.
...
Histograms are good for visualising sample distributions when we have a reasonable sample size (at least dozens, and ideally, hundreds of observations). They aren't very effective when the sample is quite small. In this 'small data' situation, it is better to use something called a **dot plot**[^univariate_eda-1].
...
Let's first use **dplyr** to extract a small subset of the `penguins` data:
...
This extracts the subset of female Adelie penguin observations from 2007. The resulting data set is considerably smaller than the original:

```{r, echo=FALSE}
penguins_subset
```

Making the dot plot with these data is simple. The **ggplot2** code is very similar to the histogram case:

```{r}
ggplot(penguins_subset, aes(x = body_mass_g)) + 
  geom_dotplot(binwidth = 100) + 
  xlab("Body mass (g)") + ylab("") + 
  theme(axis.text.y = element_blank(), axis.ticks.y = element_blank())
```

Each observation in the data adds one dot, and dots that fall into the same bin are stacked up on top of one another. Notice the bins are not evenly spaced, however. The resulting plot displays the same information as a histogram but tends to be more informative when there are relatively few observations.

The `theme` part of that example is entirely optional---it removes the grid lines and y axis labels because these are not needed. We removed the y axis label for the same reason.
...
This shows that the most common species in the data set is the Adelie, followed by the Gentoo and Chinstrap species. Gentoo is roughly as common as Adelie, whereas Chinstrap is about half as frequent.
...
Look at the layer information below `----`. The `geom_bar` function set the stat to "count". This means **ggplot2** summed the number of observations associated with each category of `species`. Counting a categorical variable is analogous to binning a numeric variable. The only difference is that there is no need to specify bin widths because `species` is categorical. Here's the resulting figure:

```{r, fig.height=4, fig.width=2.2}
plot_bar
```

This is exactly the same information produced by the `count` function, only now it's presented in graphical form. We can customise this bar graph with functions like `xlab` and `ylab`, and by setting various properties inside `geom_bar`. For example:
...
What else about this plot might we change? The different species appear in alphabetical order. There's nothing particularly meaningful about that order. Perhaps it would make more sense to show them in frequency order?
...
Is there anything else we might we change? Sometimes it is clearer to plot horizontal bars, particularly if the categorical axis labels are all bunched together (not really the case here). Flipping the x and y axes makes a horizontal bar chart, which we can achieve with the `coord_flip` function (this is new):
...
```{r, fig.height=1.8, fig.width=5}
ggplot(penguins, aes(x = as.character(year))) + 
  geom_bar(fill = "steelblue", width = 0.7) + 
  coord_flip() + 
  xlab("Year") + ylab("Number of Observations")
```

That's better! This may seem like a bit of a special edge case, but it comes up often enough to be worth highlighting the fix. More generally, this example demonstrates that we must ensure our conception of a variable matches the way it is interpreted by **ggplot2** if we want to arrive at a 'sensible' plot.
