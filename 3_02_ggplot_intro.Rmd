...
We are not going to survey all these plotting systems. It's entirely possible to meet most data visualisation needs by becoming proficient with just one of them. This book focuses on the **ggplot2** package. In many ways, **ggplot2** hits the 'sweet spot' between base graphics and **lattice**. It enables complex visualisations without the need to write many lines of R code, but flexible enough to permit the kind of customisation required to produce publication quality figures.
...
We will our work with **ggplot2** by setting up a minimal **graphical object**. That's a job for the `ggplot` function. Using `ggplot` without any arguments builds an empty graphical object:
...
Then Then print out the summary of the updated `plot_obj`:
...
One option is to map the `species` to colour so that the colour of the points correspond to different species. We do this by changing the `aes` part:

```{r, fig.width=4.5}
# display bill morphology scatter plot + colour points by species
ggplot(penguins, 
...
::: {.infobox .information data-latex="{information}"}
#### Commenting **ggplot2** code {.unnumbered}

We can place comments between the lines of **ggplot2** code. For example, we could add comments to state what the `aes` and `geom_point` parts are doing:

```{r, eval=FALSE}
# display bill morphology scatter plot
ggplot(penguins, 
       # vary colour by species
       aes(x = bill_length_mm, y = bill_depth_mm, colour = species)) + 
  # add points layer
  geom_point()
```

It's a good idea to document **ggplot2** code in this way as it starts to get more complicated. That way, when we come back to it after a while we can remember what we were trying to achieve! We will include extensive comments from now on.
:::
...
# display bill morphology scatter plot by species and island
ggplot(penguins, 
