...
This constructs the skeleton object and assigns it a name, `plot_obj`. We can use the `summary` function to inspect this:
...
The output of `summary` is quite verbose, but the important parts are near the top, just before the `faceting:` part. In this case, the 'important part' is basically empty. This tells us that there are no data, aesthetic mapping, layers, etc associated with `plot_obj`. All we did was set up an empty graphical object.
...
Notice that when we summarise the resulting object this time, we see that the variables inside `penguins` (`species`, `island`, `bill_length_mm`, ...) now comprise the data associated with the `plot_obj` object.
...
Now `plot_obj` has everything it needs to render a figure. How do we do this? Simply 'print' the object:
...
Notice that we put the `aes` argument on a new line. We did this because the first line was getting a bit long. This is just a readability thing---the `aes` part still belongs to `ggplot`.
...
It's a good idea to document **ggplot2** code in this way as it starts to get more complicated. That way, when we come back to it after a while we can remember what we were trying to achieve! We will do this more and more from now on.
:::
