...
pckg_set <- c(.packages(), 'bookdown', 'knitr', 'rmarkdown', 'packages.bib', 'readr', 'dplyr', 'ggplot2')
...
#### GUI versions of R? {-}
When we download and install R for either Windows or Mac, in addition to the basic R program discussed above, we also get another program that acts as a [Graphical User Interface](http://en.wikipedia.org/wiki/Graphical_user_interface) (GUI) for R. This is the thing labelled "R" in the Programs menu on a Windows computer or the Applications folder on a Mac. When we launch the R GUI we're presented with something called the Console, which is where we can interact directly with R by typing things at the so-called prompt, `> `, and a few buttons and menus for managing common tasks. We will not go through the GUIs in any detail because we recommend using RStudio, but it's important to be aware they exist.
:::
